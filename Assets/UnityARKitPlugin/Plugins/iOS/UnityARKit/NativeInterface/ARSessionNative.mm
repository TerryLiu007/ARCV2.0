...
// Construct s_OpenCVPixelBuffer object
static OpenCVPixelBuffer s_OpenCVPixelBuffer;
...
- (void)session:(ARSession *)session didUpdateFrame:(ARFrame *)frame
{
    ...
    if (s_OpenCVPixelBuffer.Enabled)
    {
        
        CVPixelBufferLockBaseAddress(pixelBuffer, kCVPixelBufferLock_ReadOnly);
        
        if (s_OpenCVPixelBuffer.YPixelBytes)
        {
            unsigned long numBytes = CVPixelBufferGetBytesPerRowOfPlane(pixelBuffer, 0) * CVPixelBufferGetHeightOfPlane(pixelBuffer,0);
            void* baseAddress = CVPixelBufferGetBaseAddressOfPlane(pixelBuffer,0);
            memcpy(s_OpenCVPixelBuffer.YPixelBytes, baseAddress, numBytes);
        }
...
// Create the native mirror to the C# OpenCVPixelData object
extern "C" void OpenCVPixelData (uint32_t enable, void* YPixelBytes)
{
    s_OpenCVPixelBuffer.Enabled = (BOOL) enable;
    if (s_OpenCVPixelBuffer.Enabled)
    {
        s_OpenCVPixelBuffer.YPixelBytes = YPixelBytes;
    } else {
        s_OpenCVPixelBuffer.YPixelBytes = NULL;
    }
}
